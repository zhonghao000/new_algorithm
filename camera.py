...
def shoot():
    ...
    for i in range(0, 200):
        outputs.append(['i','m','a','g','e','0','0','0','.','j','p','g'])
        if i < 10:
            outputs[i][7] = chr(int(i) + 48)
            outputs[i] = ''.join(outputs[i])
        if i >= 10 and i < 100:
            outputs[i][7] = chr(int(i % 10) + 48)
            outputs[i][6] = chr(int(i / 10) + 48)
            outputs[i] = ''.join(outputs[i])
        if i >= 100:
            outputs[i][7] = chr(int(i % 100)%10 + 48)
            outputs[i][6] = chr(int((i % 100) / 10) + 48)
            outputs[i][5] = chr(int(i / 100) + 48)
            outputs[i] = ''.join(outputs[i])
