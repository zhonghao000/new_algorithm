...
{
  int i, j, k, testTime;
  unsigned char *smoothGray = (unsigned char*) malloc(1000000 * sizeof(unsigned char));
  ...
  int bendSum, midSum;
  int classNumber[480];
  char testBuffer[50] = {'i','m','a','g','e','0','0','0','.','j','p','g'};
  //canny(smoothGray);
  ...
  while (testTime < 200){
  testBuffer[5] = testTime / 100 + '0';
  testBuffer[6] = (testTime % 100)  / 10 + '0';
  testBuffer[7] = (testTime % 100) %10 + '0';
  loadJpg(testBuffer);
  start = clock();
...
/*
    if ( minimumLimit > 70)
      minimumLimit -= 70;
    if (maximumLimit < Width - 70)
      maximumLimit += 70;
    for (i = rowBase + minimumLimit; i < rowBase + maximumLimit; i++) 
*/
    for (i = rowBase; i < rowBase + Width; i++)
    {
      if (smoothGray[i] == 200)
    ...
    if (j >= 4 )
    {
      ...
      if (classNumber[j] < 50 && j > 50 && bendSum < 50){
        state = 2;
        //printf("classNum = %d  ", classNumber[j]);
      }
